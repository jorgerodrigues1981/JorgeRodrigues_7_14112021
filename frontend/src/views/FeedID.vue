    ...
    <div v-else>
      ...
      >
        ...
        <template v-slot:postDelete v-else-if="posts[indexLastPost].yourPost > 0">
          ...
          <i class="fas fa-trash-alt" aria-hidden="true" title="Supprimer le post" role="button" v-on:click="deletePost(posts[indexLastPost].postID)"></i>
          <span class="sr-only">Supprimer le post</span>
          </div>
        </template>
      ...
      >
        <!-- Bouton supression commentaire -->
        <template v-slot:commentDelete v-if="comment.yourPost > 0">
          <div class="supprimer_post">
          <i class="fas fa-trash-alt" aria-hidden="true" title="Supprimer le commentaire" role="button" v-on:click="deletePost(comment.postID)"></i>
